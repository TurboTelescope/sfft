...
class SingleSFFTConfigure_Numpy:
    ...
    def SSCN(NX, NY, KerHW, KerPolyOrder=2, BGPolyOrder=2, ConstPhotRatio=True,
             NUM_CPU_THREADS_4SUBTRACT=8, NUMBA_CACHE=True, SINGLE_PRECISION=False, VERBOSE_LEVEL=2):
        ...
        @nb.njit(_CastNB2SinglePrecision(_strdec) if SINGLE_PRECISION else _strdec, parallel=True, cache=NUMBA_CACHE)
...
class SingleSFFTConfigure:
    ...
    def SSC(NX, NY, KerHW, KerPolyOrder=2, BGPolyOrder=2, ConstPhotRatio=True, \
        BACKEND_4SUBTRACT='Cupy', NUM_CPU_THREADS_4SUBTRACT=8, NUMBA_CACHE=True, SINGLE_PRECISION=False, VERBOSE_LEVEL=2):
        ...
        # j) SINGLE_PRECISION: Use single precision (float32/complex64) instead of double precision
        #                      (float64/complex128) throughout configuration & subtraction? (default: False)
        #
        """
        ...
        if BACKEND_4SUBTRACT == 'Numpy':
            SFFTConfig = SingleSFFTConfigure_Numpy.SSCN(NX=NX, NY=NY, KerHW=KerHW, \
                KerPolyOrder=KerPolyOrder, BGPolyOrder=BGPolyOrder, ConstPhotRatio=ConstPhotRatio, \
                NUM_CPU_THREADS_4SUBTRACT=NUM_CPU_THREADS_4SUBTRACT, NUMBA_CACHE=NUMBA_CACHE, \
                SINGLE_PRECISION=SINGLE_PRECISION, VERBOSE_LEVEL=VERBOSE_LEVEL)
