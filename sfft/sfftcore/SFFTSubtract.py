...
class ElementalSFFTSubtract_PureCupy:
    ...
    def ESSPC(PixA_I_GPU, PixA_J_GPU, SFFTConfig, SFFTSolution_GPU=None, Subtract=False, VERBOSE_LEVEL=2):
        ...
        if SFFTSolution_GPU is not None:
            assert SFFTSolution_GPU.dtype == REAL_DTYPE
        ...
        if Subtract:
            ...
            t9 = time.time()
            # Calculate Kab components
            Wl_GPU = cp.exp((-2j*cp.pi/N0) * PixA_X_GPU.astype(REAL_DTYPE))    # row index l, [0, N0)
            Wm_GPU = cp.exp((-2j*cp.pi/N1) * PixA_Y_GPU.astype(REAL_DTYPE))    # column index m, [0, N1)
            Kab_Wla_GPU = cp.empty((L0, N0, N1), dtype=COMPLEX_DTYPE)
            Kab_Wmb_GPU = cp.empty((L1, N0, N1), dtype=COMPLEX_DTYPE)
...
class GeneralSFFTSubtract_PureCupy:
    ...
    def GSS(PixA_I_GPU, PixA_J_GPU, PixA_mI_GPU, PixA_mJ_GPU, SFFTConfig, ContamMask_I_GPU=None, VERBOSE_LEVEL=2):
        ...
        if ContamMask_I_GPU is not None:
            ...
            _Solution_GPU[-Fpq:] = 0.
            REAL_DTYPE = np.float32 if SFFTConfig[0].get('SINGLE_PRECISION', False) else np.float64
            _PixA_I_GPU = ContamMask_I_GPU.astype(REAL_DTYPE)
